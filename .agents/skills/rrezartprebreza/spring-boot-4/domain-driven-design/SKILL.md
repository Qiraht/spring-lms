...
- Agent leaves nullability implicit in domain APIs — Framework 7 is JSpecify `@NullMarked`; do the same for domain packages and mark the rare nullable return with `org.jspecify.annotations.Nullable`
- Agent mocks repositories with `@MockBean` in application-service tests — removed in Boot 4; use `@MockitoBean`
