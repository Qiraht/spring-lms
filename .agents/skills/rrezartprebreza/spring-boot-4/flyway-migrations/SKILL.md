...
- Agent adds only `flyway-core` — database support lives in `flyway-database-<db>` modules; without it startup fails with "Unsupported Database"
- Migrating from Boot 3: Flyway was pulled in transitively by JDBC/JPA starters — in Boot 4 you must declare `spring-boot-starter-flyway` yourself
