// ❌ BAD — pre-Security-7 API (won't compile), wrong role check, unnecessary UserDetailsService, session-based

@Configuration
@EnableWebSecurity
public class ResourceServerConfig extends WebSecurityConfigurerAdapter {  // REMOVED — gone since Security 6, still gone in 7
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable()                             // non-lambda DSL — removed in Security 7, won't compile
            .authorizeRequests()                          // removed in Security 7 — use authorizeHttpRequests
            .antMatchers("/api/**")                       // antMatchers removed in Security 7 — use requestMatchers
            .hasRole("ADMIN")                             // hasRole expects ROLE_ prefix, not SCOPE_
            .and()                                            // and() removed from HttpSecurity in Security 7
            .oauth2ResourceServer()                       // non-lambda style removed — use lambda DSL
